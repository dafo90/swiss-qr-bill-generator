(async () => {
    ...
    const createSelectFieldElement = ({
        ...
        if (columnOptions.includes(defaultMap)) {
            selectElement.append(createOptionElement(undefined, false));
            columnOptions.map(option => selectElement.append(createOptionElement(option, option === defaultMap)));
        } else {
            selectElement.append(createOptionElement(undefined, true), ...columnOptions.map(option => createOptionElement(option)));
        }
        if (allowStaticValue) {
            selectElement.append(createOptionElement('STATIC VALUE', false));
            selectElement.onchange = () => {
                if (selectElement.value === 'STATIC VALUE') {
                    // add static value
                } else {
                    // remove static value field
                }
            };
        }
